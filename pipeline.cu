...
inline void destroy_events(std::vector<cudaEvent_t>& events) {
...
void run_feature_pipeline_async(const char* h_sequence,
    ...
    std::vector<int> chunk_offsets(num_chunks);
    std::vector<int> chunk_lengths(num_chunks);
    for (int c = 0; c < num_chunks; ++c) {
        const int offset = c * PIPELINE_CHUNK_SIZE;
        const int len = std::min(PIPELINE_CHUNK_SIZE, length - offset);
    ...
    std::vector<cudaEvent_t> ev_h2d_done(kPipelineSlots, nullptr);
    std::vector<cudaEvent_t> ev_compute_done(kPipelineSlots, nullptr);
    std::vector<cudaEvent_t> ev_d2h_done(kPipelineSlots, nullptr);
    ...
    std::vector<cudaEvent_t> h2d_start(num_chunks, nullptr);
    std::vector<cudaEvent_t> h2d_stop(num_chunks, nullptr);
    std::vector<cudaEvent_t> d2h_start(num_chunks, nullptr);
    std::vector<cudaEvent_t> d2h_stop(num_chunks, nullptr);
    std::vector<cudaEvent_t> gc_start(num_chunks, nullptr);
    std::vector<cudaEvent_t> gc_stop(num_chunks, nullptr);
    std::vector<cudaEvent_t> homo_start(num_chunks, nullptr);
    std::vector<cudaEvent_t> homo_stop(num_chunks, nullptr);
    std::vector<cudaEvent_t> motif_start(num_chunks, nullptr);
    std::vector<cudaEvent_t> motif_stop(num_chunks, nullptr);
    std::vector<cudaEvent_t> entropy_start(num_chunks, nullptr);
    std::vector<cudaEvent_t> entropy_stop(num_chunks, nullptr);
    ...
    for (int step = 0; step < num_chunks + 2; ++step) {
        ...
        if (load_chunk < num_chunks) {
            ...
            std::memcpy(h_input_stage[slot], h_sequence + offset, static_cast<size_t>(len) * sizeof(char));
    ...
    if (h_result->gc_violations == nullptr) {
        ...
        std::memcpy(h_result->gc_violations, h_gc_pinned, static_cast<size_t>(length) * sizeof(int));
    ...
    if (h_result->homopolymer_violations == nullptr) {
        ...
        std::memcpy(h_result->homopolymer_violations,
                    h_homo_pinned,
    ...
    if (h_result->motif_collisions == nullptr) {
        ...
        std::memcpy(h_result->motif_collisions, h_motif_pinned, static_cast<size_t>(length) * sizeof(int));
    ...
    if (h_result->entropy_vector == nullptr) {
        ...
        std::memcpy(h_result->entropy_vector, h_entropy_pinned, static_cast<size_t>(length) * sizeof(float));
